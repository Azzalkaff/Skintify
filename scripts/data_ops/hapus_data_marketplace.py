...
def hapus_data_marketplace():
    """Menghapus semua data Tokopedia dan Lazada dari database."""
    session = SessionLocal()
    try:
        print("[*] Memulai pembersihan database...")
        
        # 1. Hapus Produk
        produk_terhapus = session.query(Produk).filter(Produk.platform.in_(['tokopedia', 'lazada'])).delete(synchronize_session=False)
        print(f"[OK] Berhasil menghapus {produk_terhapus} data produk.")
        
        # 2. Hapus Toko
        toko_terhapus = session.query(Toko).filter(Toko.platform.in_(['tokopedia', 'lazada'])).delete(synchronize_session=False)
        print(f"[OK] Berhasil menghapus {toko_terhapus} data toko.")
        
        # 3. Hapus Hasil Pencarian
        pencarian_terhapus = session.query(HasilPencarian).filter(HasilPencarian.platform.in_(['tokopedia', 'lazada'])).delete(synchronize_session=False)
        print(f"[OK] Berhasil menghapus {pencarian_terhapus} riwayat pencarian.")
        
        session.commit()
        print("\n[DONE] Database Tokopedia dan Lazada telah dikosongkan!")
